...
export default function CardProduto(props) {
    ...
    const addFoodCart = (food) => {
        ...
        if (localStorage.getItem("buyFood")){
            ...
            if(valueFood > -1){
              arrayBuyFood[valueFood].count += Number(count)
      ...
      const RemoveFoodCart = (food)=>{
          setCount(0)
        const arrayBuyFood  = JSON.parse(localStorage.getItem("buyFood"))
        const newArrayFood = arrayBuyFood.filter(item=>{
            return item.id !== food.id
        })
        localStorage.setItem("buyFood", JSON.stringify(newArrayFood))
      }
  ...
  return (
    <Container >
        ...
        <AddButton onClick={()=> setSelect(true)} display={display}>Adicionar</AddButton>  
        <RemoveButton onClick={()=> RemoveFoodCart(props.food)}  display={display}>Remover</RemoveButton> 

        <dialog open={select} >
            <Input type='number' value={count} onChange={e=> setCount(e.target.value)}/>
            <Button onClick={()=> addFoodCart(props.food)}>adicionar pedido</Button>
        </dialog >
    </Container>
  );
}
